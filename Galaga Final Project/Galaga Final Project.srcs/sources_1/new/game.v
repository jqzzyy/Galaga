...
module game(
    ...
    // amount of enemies that will spawn
    ...
    //levels, for each time all enemies are cleared the enemy bullet speed will increase by 2
    reg [1:0] level;
    ...
    // enemy instances 32 w/ unique seeds and staggered timers
    ...
    // enemy pixel mux 
    //scan all, last active pixel wins
    ...
    reg all_dead_prev;
    reg  all_dead_r; // combinational check across all enemies
    integer ad;
    always @(*) begin
        all_dead_r = 1;
    ...
    always @(posedge frame_tick) begin
        if (rst) begin
            ...
            // respawn grid when all enemies are dead on posedge
            all_dead_prev <= all_dead;
            if (all_dead && !all_dead_prev) begin
                for (i = 0; i < ENEMY_COUNT; i = i + 1) begin
                    enemy_alive[i] <= 1;
                end
                    level <= level + 1;
                    ebullet_speed <= ebullet_speed + 2;
            end
            ...
            if (scanline_count < 524) begin
                ...
                // player bullet collision w/ enemies
